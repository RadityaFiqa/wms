...
let ErpDocumentReferenceService = ErpDocumentReferenceService_1 = class ErpDocumentReferenceService {
    ...
    PO_PICKING_TYPE = 11852;
    SO_PICKING_TYPE = 11849;
    ...
    async executeSyncJob(warehouseId, logId, triggeredBy) {
        ...
        try {
            ...
            while (iterations < 100) {
                ...
                if (!hasNew || records.length < limit)
                    break;
                offset += limit;
            }
        ...
        try {
            for (let i = 0; i < totalDocuments; i += batchSize) {
                ...
                this.logger.log(`[SYNC-JOB] Batch ${Math.floor(i / batchSize) + 1}: ${batchDocs.length} docs, ` +
