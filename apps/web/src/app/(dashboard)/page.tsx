...
export default function DashboardOverviewPage() {
  ...
  // Define modules with icons, descriptions, and permission checks
  const modules = [
    {
      name: 'Inventory',
  ...
  // Frequently used / priority modules first
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-12">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-3xl p-8 md:p-10 text-white shadow-xl relative overflow-hidden">
        <div className="relative z-10 max-w-2xl">
          ...
          <p className="mt-3 text-blue-100 text-sm md:text-base leading-relaxed">
            Selamat datang di WMS BULOG. Anda masuk dengan wewenang <strong className="text-white bg-white/10 px-2 py-0.5 rounded-md text-xs">{user?.role}</strong>. Silakan pilih salah satu modul di bawah ini untuk memulai pekerjaan Anda.
          </p>
        </div>
      ...
      <div className="space-y-4">
        <div>
          <h2 className="text-lg font-black text-slate-800 dark:text-slate-100 tracking-tight">Modul Operasional & Administrasi</h2>
          <p className="text-xs text-slate-500 mt-0.5">Daftar modul yang dapat Anda akses berdasarkan peran (roles & permissions) Anda.</p>
        </div>

        {sortedModules.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center text-slate-400 font-medium">
            Anda tidak memiliki izin untuk mengakses modul apapun. Silakan hubungi Administrator.
          </div>
        ) : (
            ...
            {sortedModules.map((mod) => {
              ...
              return (
                ...
                >
                  <div className="space-y-4">
                    {/* Module Icon */}
                    <div className={`p-3 rounded-xl border w-fit shrink-0 ${iconWrapperClasses}`}>
                      <Icon className="h-6 w-6" />
                    </div>

                    {/* Name & Description */}
                    <div className="space-y-1.5">
                      <h3 className={`font-bold flex items-center gap-1.5 text-base ${titleClasses}`}>
                        {mod.name}
                        {mod.priority && (
                      ...
                      <p className={`text-xs leading-relaxed line-clamp-3 ${descClasses}`}>
                        {mod.description}
                      </p>
                    </div>
                  </div>
