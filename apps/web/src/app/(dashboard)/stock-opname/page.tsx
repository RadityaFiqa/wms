...
export default function StockOpnameListPage() {
  ...
  const { stockOpnameData, isLoading, refresh, createStockOpname } =
    useStockOpname({
      search,
      status,
      startDate,
  ...
  const handleCreate = async () => {
    ...
    const toastId = toast.loading(
      "Membuat sesi Stock Opname baru & mengambil snapshot stok...",
    );
    try {
      const res = await createStockOpname(
        "Sesi Opname " + activeWarehouse.name,
      );
      toast.success("Sesi Stock Opname berhasil dibuat!", { id: toastId });
      router.push(`/stock-opname/${res.uuid}`);
    } catch (e: any) {
      toast.error(e.response?.data?.message || "Gagal membuat Stock Opname.", {
        id: toastId,
      });
    } finally {
      setIsCreating(false);
          ...
          <table className="w-full text-left border-collapse table-layout-fixed">
            <thead className="bg-slate-50 dark:bg-slate-850/80 border-b border-slate-200 dark:border-slate-800">
              <tr className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                ...
                <th className="px-6 py-4 w-[13%] text-right font-semibold text-amber-700 dark:text-amber-400">
                  Selisih Var
                </th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
              {isLoading ? (
                Array.from({ length: limit }).map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="h-4 bg-slate-250 dark:bg-slate-800 rounded w-8 mx-auto"></div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="h-4 bg-slate-250 dark:bg-slate-800 rounded w-12 ml-auto"></div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="h-4 bg-slate-250 dark:bg-slate-800 rounded w-12 mx-auto"></div>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <div className="h-4 bg-slate-250 dark:bg-slate-800 rounded w-16 mx-auto"></div>
                    </td>
                  </tr>
                ))
                ...
                <tr>
                  ...
                  >
                    Tidak ada sesi Stock Opname ditemukan. Silakan klik tombol
                    Mulai Opname Baru.
                  </td>
                </tr>
              ) : (
                stockOpnameData.items.map((op: any) => (
                  <tr
                    key={op.uuid}
                    className="hover:bg-slate-50/50 dark:hover:bg-slate-800/20 border-b border-slate-100 dark:border-slate-800/80 transition text-slate-700 dark:text-slate-350"
                  >
                    ...
                    <td className="px-6 py-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        {op.status === "DRAFT" ? (
                          <button
                            onClick={() =>
                              router.push(`/stock-opname/${op.uuid}`)
                            }
        ...
        {stockOpnameData?.totalPages > 1 && (
          <div className="bg-slate-50 dark:bg-slate-850/80 px-4 sm:px-6 py-4 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-xs text-slate-500 font-semibold text-center sm:text-left">
              Menampilkan {stockOpnameData.items.length} dari{" "}
              {stockOpnameData.total} data opname
            </span>
