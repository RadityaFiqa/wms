...
export default function StockOpnameDetailPage() {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 space-y-3">
        ...
        <span className="text-sm font-semibold text-slate-500">Memuat detail Stock Opname...</span>
      </div>
    );
  }
  ...
  if (error || !detailData) {
    return (
      <div className="bg-red-50 text-red-700 p-6 rounded-2xl border border-red-150 space-y-4">
        ...
        <p>{error?.response?.data?.message || 'Sesi Stock Opname tidak ditemukan.'}</p>
        <button onClick={() => router.push('/stock-opname')} className="px-4 py-2 bg-red-600 text-white rounded-lg font-semibold hover:bg-red-500 transition cursor-pointer">
          Kembali ke Daftar
        </button>
      </div>
    );
  ...
  const buildSavePayload = () => {
    const stacksPayload = Object.entries(actualCounts).map(([stackUuid, val]) => ({
      uuid: stackUuid,
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      toast.error(e.response?.data?.message || 'Gagal menyimpan draf.', { id: toastId });
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSubmitOpname = async () => {
    ...
    const toastId = toast.loading('Menyimpan data dan menyelesaikan Stock Opname...');
    try {
      await updateDraft(buildSavePayload());
      await submitOpname();
      toast.success('Stock Opname berhasil diselesaikan dan dikunci.', { id: toastId });
    } catch (e: any) {
      toast.error(e.response?.data?.message || 'Gagal menyelesaikan Stock Opname.', { id: toastId });
    } finally {
  ...
  const handleExportPdf = async () => {
    const toastId = toast.loading(isDraft ? 'Mengunduh Counting Sheet...' : 'Mengunduh Laporan Stock Opname...');
    try {
      if (isDraft) {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-2xl shadow-xs gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="font-mono text-xl md:text-2xl font-black text-slate-850 dark:text-slate-100">{detailData.opnameNumber}</span>
            {isDraft ? (
          ...
          <div className="text-xs text-slate-500 flex flex-wrap items-center gap-x-4 gap-y-1">
            <span className="flex items-center gap-1">
              ...
              Mulai: {new Date(detailData.createdAt).toLocaleString('id-ID', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
            </span>
            {detailData.completionDate && (
              <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
                <CheckCircle className="h-3.5 w-3.5 shrink-0" />
                Selesai: {new Date(detailData.completionDate).toLocaleString('id-ID', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
              </span>
            )}
      ...
      {isDraft && (
        <div className="bg-amber-50/50 dark:bg-amber-950/10 border border-amber-100 dark:border-amber-900/30 rounded-2xl p-4 flex items-start gap-3.5 shadow-xs">
          ...
          <div className="text-xs text-amber-800 dark:text-amber-300 leading-relaxed">
            <span className="font-bold">Panduan Counting:</span> Klik pada baris produk di tabel untuk menampilkan rincian tumpukan/lokasi. Masukkan kuantitas aktual yang Anda hitung. Kolom input yang dibiarkan kosong akan diabaikan dan tidak dimasukkan ke dalam perhitungan selisih/varian.
          </div>
        </div>
      )}
          ...
          <table className="w-full text-left border-collapse table-layout-fixed">
            <thead className="bg-slate-50 dark:bg-slate-850/80 border-b border-slate-200 dark:border-slate-800">
              <tr className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                ...
                <th className="px-6 py-3.5 text-right w-[18%]">ERP Stock Snapshot</th>
                <th className="px-6 py-3.5 text-right w-[18%]">Realtime Stock Snapshot</th>
                <th className="px-6 py-3.5 text-right w-[18%]">Perhitungan Aktual</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-sm">
              {detailData.items.map((item: any) => {
                ...
                const diff = !isDraft ? item.difference : (hasActualVal ? actualSum - item.erpStock : null);
                ...
                return (
                  <React.Fragment key={item.uuid}>
                    ...
                    >
                      <td className="px-6 py-4 font-bold text-slate-800 dark:text-slate-200 flex items-center space-x-2">
                        ...
                        <div className="truncate">
                          ...
                          <div className="text-slate-400 font-mono text-[10px] mt-0.5">{item.productSku}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-slate-700 dark:text-slate-300">
                        {item.erpStock.toLocaleString('id-ID')} <span className="text-slate-400 text-xs font-normal ml-0.5">{item.productUom}</span>
                      </td>
                      <td className="px-6 py-4 text-right font-bold text-slate-500 dark:text-slate-400">
                        {item.realtimeStock.toLocaleString('id-ID')} <span className="text-slate-400 text-xs font-normal ml-0.5">{item.productUom}</span>
                      </td>
                      <td className="px-6 py-4 text-right font-extrabold text-slate-800 dark:text-slate-200">
                        {hasActualVal ? (
                          <span>
                            {actualSum.toLocaleString('id-ID')}{' '}
                            <span className="text-slate-400 text-xs font-normal ml-0.5">{item.productUom}</span>
                          </span>
                        ) : (
                          <span className="text-slate-350 dark:text-slate-650 italic font-normal text-xs">Belum dihitung</span>
                        )}
                      </td>
                    ...
                    {isExpanded && (
                      <tr>
                        <td colSpan={5} className="bg-slate-50/50 dark:bg-slate-900/20 px-8 py-4 border-b border-slate-200 dark:border-slate-800">
                          <div className="max-w-2xl bg-white dark:bg-slate-850 border border-slate-200/80 dark:border-slate-800/80 rounded-xl p-4 space-y-3.5 shadow-xs">
                            ...
                            <div className="divide-y divide-slate-100 dark:divide-slate-800">
                              {item.stacks.map((stack: any) => {
                                ...
                                const stackVar = (inputVal !== undefined && inputVal !== '') ? Number(inputVal) - stack.erpQty : null;

                                return (
                                  <div key={stack.uuid} className="py-2.5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-xs sm:text-sm">
                                    <div className="space-y-0.5">
                                      <div className="font-bold text-slate-800 dark:text-slate-200">{stack.locationName}</div>
                                      <div className="text-[11px] text-slate-400">Qty Snapshot ERP: <strong className="font-bold text-slate-600 dark:text-slate-400">{stack.erpQty.toLocaleString('id-ID')} {item.productUom}</strong></div>
                                    </div>

                                    <div className="flex items-center gap-4">
                                      {/* Input */}
                                      <div className="flex items-center gap-2">
                                        <label className="text-xs text-slate-400 font-medium">Aktual:</label>
                                        <input
                                          type="text"
                                          disabled={!isDraft}
                                          value={isDraft ? inputVal : (stack.actualQty !== null ? stack.actualQty : '')}
                                          onChange={(e) => handleInputChange(stack.uuid, e.target.value)}
                                        ...
                                        />
                                        <span className="text-xs text-slate-450">{item.productUom}</span>
                                      </div>
                                      ...
                                      <div className="w-16 text-right font-black">
                                        {isDraft ? (
                                          stackVar !== null ? (
                                            <span className={stackVar < 0 ? 'text-red-650' : stackVar > 0 ? 'text-emerald-650' : 'text-slate-500'}>
                                              {stackVar > 0 ? `+${stackVar}` : stackVar}
                                            </span>
                                          ) : (
                                          ...
                                          stack.variance !== null ? (
                                            <span className={stack.variance < 0 ? 'text-red-650' : stack.variance > 0 ? 'text-emerald-650' : 'text-slate-500'}>
                                              {stack.variance > 0 ? `+${stack.variance}` : stack.variance}
                                            </span>
                                          ) : (
                                            <span className="text-slate-300">-</span>
                                          )
            ...
            <div className="space-y-2">
              {detailData.attachments.length === 0 ? (
                ...
                detailData.attachments.map((attach: any) => (
                  ...
                  >
                    <span className="truncate max-w-[200px]">{attach.fileName}</span>
