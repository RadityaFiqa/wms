    ...
    return (
      <div className="bg-white border border-slate-200 rounded-xl p-8 text-center shadow-sm space-y-4 max-w-lg mx-auto">
        ...
        <p className="text-sm text-slate-500">
          Data gate operation dengan UUID tersebut tidak valid atau tidak dapat
          diakses.
        </p>
        <button
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {(gateOperation.status === "PENDING" ||
            gateOperation.status === "PARTIAL") &&
            hasPermission("create", "GateVerification") && (
              <Link
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4 text-xs">
            ...
            <div>
              ...
              <p className="text-sm font-semibold text-slate-700 mt-1">
                ...
                {gateOperation.createdByUser?.email})
              </p>
            </div>

            <div>
              <p className="font-bold text-slate-400 uppercase tracking-wider">
                Dokumen Referensi ERP
              </p>
                  ...
                  <>
                    ...
                    {gateOperation.documentHistory && (
                      <button
                        ...
                        className="text-blue-600 hover:text-blue-700 font-bold text-[10px] mt-1 flex items-center hover:underline cursor-pointer"
                      >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-t border-slate-100 pt-4">
              {gateOperation.poReferences?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {gateOperation.poReferences.map(
                      (po: string, index: number) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-blue-50 text-blue-700 border border-blue-150 font-mono"
                        >
                          {po}
                        </span>
                      ),
              ...
              {gateOperation.soReferences?.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {gateOperation.soReferences.map(
                      (so: string, index: number) => (
                        <span
                          key={index}
                          className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold bg-purple-50 text-purple-700 border border-purple-150 font-mono"
                        >
                          {so}
                        </span>
                      ),
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                {gateOperation?.products?.map((p: any) => {
                  ...
                  const locationName =
                    p?.location?.displayName || p?.locationDisplayName || "-";
                  const { location: parentLocation, stack } =
                    parseLocationName(locationName);
                  ...
                  const docItem = gateOperation.documentReference?.items?.find(
                    (item: any) => item.inventoryId === p.inventoryId,
                  );
                  const erpQty = docItem
                    ? (docItem.productQty || docItem.quantity).toLocaleString("id-ID")
                    : "-";

                  return (
                    <tr
                      key={p.uuid}
                      className="hover:bg-slate-50/20 transition-all duration-200"
                    >
      ...
      {gateOperation.verification &&
        (() => {
          ...
          return (
            <div
              className={`bg-white border border-slate-250 rounded-xl p-6 shadow-sm space-y-5 ${getBorderColorClass(gateOperation.verification.status)}`}
            >
              <h3 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-3 flex items-center">
              ...
              {gateOperation.verification.attachments &&
                gateOperation.verification.attachments.length > 0 && (
                  <div className="pt-2 border-t border-slate-100">
