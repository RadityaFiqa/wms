...
export default function RoleManagementPage() {
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      toast.success(
        `Izin akses untuk role ${selectedRole.name} berhasil diperbarui.`,
      );
      setIsEditOpen(false);
    } catch (error: any) {
      toast.error(
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">
          Konfigurasi pembagian hak akses (*Permission*) untuk setiap peran
          pengguna (*Role*) di sistem WMS.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {rolesLoading
          ? Array.from({ length: 4 }).map((_, i) => (
              <div
                key={i}
                className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm animate-pulse h-48"
              ></div>
            ))
          : roles?.map((role: any) => (
              <div
          ...
          <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl border border-slate-100 flex flex-col max-h-[85vh] animate-zoom-in">
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              <p className="text-sm font-semibold text-slate-600">
                Pilih izin akses yang ingin diberikan ke role ini:
              </p>
