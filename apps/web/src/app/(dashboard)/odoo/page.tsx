...
export default function OdooConfigPage() {
  ...
  const onSubmit = async (data: any) => {
    ...
    if (!activeConfig && (!data.password || data.password.length < 4)) {
      toast.error(
        "Kredensial Password Odoo minimal 4 karakter wajib diisi untuk konfigurasi baru.",
      );
      return;
    }
    ...
    try {
      if (activeConfig) {
        ...
        toast.success("Konfigurasi ERP Odoo berhasil diperbarui.", {
          id: toastId,
        });
      } else {
        const payload = {
        ...
        toast.success("Konfigurasi ERP Odoo berhasil disimpan.", {
          id: toastId,
        });
      }
    } catch (error: any) {
  ...
  const handleManualRefresh = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Gagal menyegarkan sesi Odoo.",
        { id: toastId },
      );
    } finally {
  ...
  const handleToggleActive = async () => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Gagal mengubah status aktif.",
        { id: toastId },
      );
    }
  };
  ...
  const getSessionStatusBadge = () => {
    ...
    const isExpired =
      ...
      new Date(activeConfig.sessionExpiredAt) <= new Date();
    if (isExpired) {
      return (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border-l-4 border-blue-500 p-4 rounded-xl text-sm text-blue-800 leading-relaxed shadow-sm">
        💡 <strong>Info Sistem</strong>: Sesi Odoo otomatis dijaga agar tetap
        aktif. Sistem menjalankan background scheduler harian untuk menyegarkan
        token sebelum kedaluwarsa (Masa berlaku sesi Odoo adalah 7 hari).
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm lg:col-span-2">
          <div className="flex items-center space-x-3 border-b border-slate-100 pb-4 mb-6">
            ...
            <h3 className="text-lg font-bold text-slate-800">
              {activeConfig
                ? "Kredensial Akun Odoo"
                : "Hubungkan ERP Odoo Baru"}
            </h3>
          </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                <label
                  htmlFor="password"
                  className="block text-sm font-semibold text-slate-700 mb-2 flex justify-between"
                >
