...
export default function UserManagementPage() {
  ...
  const handleResetPassword = async (user: any) => {
    if (!confirm(`Apakah Anda yakin ingin mereset password user ${user.name}? Password baru yang aman akan dihasilkan secara acak dan dikirimkan ke email mereka.`)) {
      return;
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-4 sm:space-y-0">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-800 tracking-tight">Manajemen User</h1>
          <p className="text-slate-500 mt-1">Daftar pengguna aplikasi WMS, alokasi gudang, dan peran akses mereka.</p>
        </div>
        <button
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
          <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Cari Nama/Email</label>
        ...
        <div>
          <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Filter Peran (Role)</label>
          <select
            value={roleId}
            onChange={(e) => {
        ...
        <div>
          <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Filter Status</label>
          <select
            value={isActive}
            onChange={(e) => {
        ...
        {usersData?.meta && (
          <div className="bg-slate-50 px-4 sm:px-6 py-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
            <span className="text-xs text-slate-500 font-semibold text-center sm:text-left">
              Menampilkan {usersData.data.length} dari {usersData.meta.total} user
            </span>
            <div className="flex items-center space-x-2">
              ...
              <span className="text-sm font-bold text-slate-700">{page} / {usersData.meta.totalPages}</span>
              <button
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl border border-slate-100 animate-zoom-in">
            ...
            <form onSubmit={handleCreateSubmit(onCreateUser)} className="p-6 space-y-4">
              <div>
                ...
                />
                {createErrors.name && (
                  <p className="text-xs text-red-500 mt-1">{createErrors.name.message}</p>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {createErrors.roleId && (
                    <p className="text-xs text-red-500 mt-1">{createErrors.roleId.message}</p>
                  )}
                </div>

                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Gudang</label>
                  <select
            ...
            <form onSubmit={handleEditSubmit(onEditUser)} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  {editErrors.roleId && (
                    <p className="text-xs text-red-500 mt-1">{editErrors.roleId.message}</p>
                  )}
                </div>
