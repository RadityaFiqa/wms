...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const menuItems = [
    ...
    },
    {
      ...
      href: '/dashboard/gate-operations',
      icon: Truck,
  ...
  const inactiveLinkClass = 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-slate-100';
  ...
  const sidebarContent = (
    <div className="flex flex-col h-full bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800">
      ...
      <div className="px-6 py-5 border-b border-slate-100 dark:border-slate-800/65 bg-slate-50/50 dark:bg-slate-900/30">
        <div className="font-bold text-slate-800 dark:text-slate-200 leading-tight">{user.name}</div>
        <div className="text-xs font-semibold text-blue-600 dark:text-blue-400 mt-0.5 mb-3">{user.role}</div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <select
                ...
                onChange={(e) => {
                  const selectedWh = user.accessibleWarehouses.find(w => w.uuid === e.target.value);
                  if (selectedWh) {
                  ...
                  <div className="p-1 space-y-0.5">
                    ...
                    >
                      <div className="flex items-center">
                        {theme === 'dark' ? (
                          <>
                            <svg className="h-4 w-4 mr-2.5 text-amber-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          ...
                          <>
                            <svg className="h-4 w-4 mr-2.5 text-indigo-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <div className={`w-8.5 h-5 flex items-center rounded-full p-0.5 transition duration-300 ${theme === 'dark' ? 'bg-blue-600' : 'bg-slate-350'}`}>
                        <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition duration-300 ${theme === 'dark' ? 'translate-x-3.5' : 'translate-x-0'}`} />
                      </div>
                    </button>
                  </div>
