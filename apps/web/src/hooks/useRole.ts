...
export function useRole() {
  ...
  const { data: permissions, error: permissionsError, isLoading: permissionsLoading } = useSWR(
    API_ROUTES.role.permissions,
    (url) => api.get(url).then((res) => res.data)
  );
  ...
  const updateRole = useCallback(async (uuid: string, payload: any) => {
