...
export function useGate() {
  ...
  const refreshList = useCallback(() => {
    mutate((key) => typeof key === 'string' && key.startsWith(API_ROUTES.gateOperations.list));
  }, []);
  ...
  const createGateOperation = useCallback(async (payload: any) => {
    const res = await api.post(API_ROUTES.gateOperations.create, payload);
    refreshList();
    return res.data;
  }, [refreshList]);

  const verifyGateOperation = useCallback(async (operationUuid: string, payload: any) => {
    const res = await api.post(API_ROUTES.gateVerifications.verify(operationUuid), payload);
    refreshList();
...
export function useGateOperations(query: {
  search?: string;
  cardType?: string;
  status?: string;
  startDate?: string;
