...
export function useErpDocuments(query?: {
  ...
  if (query?.gateOperationUuid) searchParams.append("gateOperationUuid", query.gateOperationUuid);
...
export function useErpSyncStatus() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ErpSyncStatusResponse>(
    ...
    () =>
      api
        .get(API_ROUTES.erpDocumentReferences.syncStatus)
        .then((res) => res.data),
  );
