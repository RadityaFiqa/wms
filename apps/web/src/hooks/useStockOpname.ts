...
export function useStockOpname(query?: {
  ...
  const refresh = useCallback(() => {
    mutate(
      (key) =>
        typeof key === "string" && key.startsWith(API_ROUTES.stockOpname.list),
    );
  }, []);

  const createStockOpname = useCallback(
    async (notes?: string) => {
...
export function useStockOpnameDetail(uuid?: string) {
  ...
  } = useSWR(uuid ? API_ROUTES.stockOpname.detail(uuid) : null, (url) =>
  ...
  const updateDraft = useCallback(
    async (body: {
      ...
      mutate(
        (key) =>
          typeof key === "string" &&
          key.startsWith(API_ROUTES.stockOpname.list),
      );
      return res.data;
    },
    [uuid, refreshDetail],
